...
def run_forward_test(config: ExperimentConfig, methods: list[str] | None = None) -> None:
    selected_methods = methods or ["fd_hjb_viscosity"] + [f"nn_{name}" for name in NN_ARCHITECTURES]
        ...
        for n_assets in run_config.problem.asset_counts:
            ...
            for seed in run_config.problem.seed_list:
                for method in selected_methods:
                    ...
                    solver_result.metadata["evaluation_mode"] = "forward_test"
                    solver_result.metadata["test_observations"] = int(len(split.test_returns))
                    ...
                    evaluation_result = evaluate_solver_forward_test(solver, split.test_returns, run_config)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run forward tests on unseen real test data.")
    parser.add_argument("--methods", nargs="*", default=None, help="Optional subset of methods to run.")
    parser.add_argument("--results-dir", type=Path, default=Path("results/forward_test"))
    parser.add_argument("--nn-device", default="auto", choices=["auto", "cpu", "mps", "cuda"])
    parser.add_argument(
        "--rebalance-frequency",
        default=None,
    ...
    parser.add_argument("--target-wealth", type=float, default=1.0)
    parser.add_argument("--initial-wealths", type=float, nargs="*", default=None)
    parser.add_argument("--asset-counts", type=int, nargs="*", default=None)
    parser.add_argument("--seed-list", type=int, nargs="*", default=None)
    parser.add_argument("--n-wealth-points", type=int, default=None)
    parser.add_argument("--horizon-steps", type=int, default=None)
    parser.add_argument("--n-control-samples", type=int, default=None)
    parser.add_argument("--nn-hidden-dim", type=int, default=None)
    parser.add_argument("--nn-depth", type=int, default=None)
    parser.add_argument("--nn-epochs", type=int, default=None)
    parser.add_argument("--nn-train-paths", type=int, default=None)
    parser.add_argument("--nn-validation-paths", type=int, default=None)
    parser.add_argument("--nn-batch-size", type=int, default=None)
    parser.add_argument("--nn-learning-rate", type=float, default=None)
    parser.add_argument("--nn-path-sampler", default=None, choices=["bootstrap", "gaussian"])
    parser.add_argument("--n-eval-paths", type=int, default=None)
...
def main() -> None:
    ...
    run_forward_test(config, args.methods)
